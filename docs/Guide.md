...
From the hamburger menu at the end of the row of templates, you can _Save_ your guide setup to a file or _Open_ a previously made one.
...
In the next row is a preview swatch and picker for the color of the guide. All the default guides have transparency.
...
| A | Angle switch. Non-90° guides are on the wish-list, but for now, only horizontal and vertical guides are supported. |

| Button | Decription |
|:-:| |
| **+** | Adds the current configuration as a guide. If you currently have a guide sleected in the list, that guide is updated. |
